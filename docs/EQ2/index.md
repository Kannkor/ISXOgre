# EverQuest 2 - ISXOgre

Welcome to the EQ2 section of ISXOgre documentation. Here you'll find everything you need to get started with OgreBot, configure it for your playstyle, and dive into advanced scripting.

---

## Quick Links

### New Users

- [Why OgreBot?](getting-started/why-ogrebot.md) - What makes OgreBot the best choice
- [Pricing](about/pricing.md) - Subscription information
- [Installation](getting-started/installation.md) - Get up and running
- [First Run](getting-started/first-run.md) - Loading OgreBot for the first time
- [FAQ](getting-started/faq.md) - Frequently asked questions

### OgreBot Users

- [Tabs Overview](ogrebot/tabs/index.md) - All OgreBot UI tabs explained
- [Aliases](ogrebot/aliases.md) - Target aliases (@Tank, @Healer, etc.)
- [Profiles](ogrebot/profiles.md) - Save and load configurations
- [Instance Controller](ogrebot/instance-controller.md) - Automated instance running
- [Encounter Coding](encounter-coding/index.md) - Fight-specific automation modules

### Tools

- [OgreCraft](tools/ogrecraft.md) - Automated crafting
- [Harvesting](tools/harvesting.md) - Automated resource gathering
- [Inventory Manager](tools/inventory-manager.md) - Inventory automation
- [Collection Manager](tools/collection-manager.md) - Collection tracking
- [Auto Login](tools/auto-login.md) - Automated character login
- [OgreMapper](tools/ogre-mapper.md) - Zone mapping and waypoints

### Developers

- [OgreBotAPI Reference](developer/ogrebot-api.md) - Complete API for script developers
- [CampSpot System](developer/camp-spot.md) - Positioning system
- [Coding Practices](developer/coding-practices.md) - Conventions and best practices

### Community

- [OgreIRC](community/ogreirc.md) - IRC bridge for cross-player bot control
- [Contact](community/contact.md) - How to reach us

### About

- [Changelog](about/changelog.md) - Version history and patch notes
