...
| Command | Description |
|---------|-------------|
| `-CampSpot <who> [close] [far]` | Set campspot at current location (alias: `-cs`) |
| `-ChangeCampSpotWho <who> X Y Z` | Move campspot to coordinates |
| `-ccsw <who> X Y Z` | Alias for ChangeCampSpotWho |
| `-CS_Set_Relative <who> X Y Z` | Set relative offset |
| `-CS_Set_ChangeCampSpotBy <who> X Y Z` | Add offset to campspot |
| `-ChangeCampSpotBy <who> X Y Z` | Alias for above |
| `-CS_ClearCampSpot <who>` | Clear campspot |
| `-CS_ClearRelative <who>` | Clear relative offset only |
| `-CS_ClearAll <who>` | Clear everything |
| `-CS_HowClose <who> <distance>` | Set minimum distance |
| `-CS_HowFar <who> <distance>` | Set maximum distance |
| `-CS_Set_Formation_Circle <who> ...` | Circle formation |
| `-CS_Set_Formation_MonkeyInMiddle <who> ...` | One in middle formation |
| `-CS_Set_Formation_MonkeyInMiddleRaid <who> ...` | Raid formation with configurable spots |
| `-SetCS_PositionNPC <who> <name/ID> [dist] [skip]` | Fighters in front, others behind NPC |
| `-SetCS_InFrontNPC <who> <name/ID> [dist] [skip]` | Everyone in front of NPC (180 degrees) |
| `-SetCS_BehindNPC <who> <name/ID> [dist] [skip]` | Everyone behind NPC (0 degrees) |
| `-SetCS_NPC <who> <angle> <name/ID> [dist] [skip]` | Custom angle relative to NPC |

### Local Object (Ogre_CampSpot) Methods

| Method | Description |
|--------|-------------|
| `:Set_CampSpot[who, close, far]` | Set at current location |
| `:Set_CS[who, close, far]` | Alias for Set_CampSpot |
| `:Set_CCS[X, Y, Z]` | Change to coordinates (local) |
| `:Set_ChangeCampSpot[who, X, Y, Z]` | Change with targeting |
| `:Set_Relative[who, X, Y, Z]` | Set relative offset |
| `:ClearCampSpot[who]` | Clear campspot |
| `:ClearRelative[who]` | Clear relative only |
| `:ClearAll[who]` | Clear everything |
| `:Disable[reason]` | Disable with named reason |
| `:Enable[reason]` | Re-enable (remove reason) |
| `:Set_HowClose[who, dist]` | Set minimum distance |
| `:Set_HowFar[who, dist]` | Set maximum distance |

### Local Object (Ogre_CampSpot) Members

| Member | Description |
|--------|-------------|
| `.AtCampSpot` | TRUE if within HowClose of Get_CampSpot |
| `.Get_CampSpot` | Where character moves to (relative if set, otherwise absolute) |
| `.Get_AbsoluteCampSpot` | Primary campspot (before relative offset) |
| `.Get_RelativeMods` | The relative offset values (0,0,0 when no offset) |
| `.Get_X`, `.Get_Y`, `.Get_Z` | Individual coordinates of Get_CampSpot |
| `.Get_HowClose`, `.Get_HowFar` | Current HowClose and HowFar values |
| `.DistanceFromCampSpot` | Distance from Get_CampSpot |
| `.DistanceFromAbsoluteCampSpot` | Distance from Get_AbsoluteCampSpot |
| `.Disabled[reason]` | TRUE if disabled by named reason |
| `.UsingRelative` | TRUE if Get_CampSpot differs from Get_AbsoluteCampSpot |
